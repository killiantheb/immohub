...
# ── Parse-location (ex sphere_carte.py — fusionné Lot C) ──────────────────────
...
_PARSE_LOCATION_CITY_FALLBACK: dict[str, tuple[float, float]] = {
    "genève":    (6.143, 46.204),
...
_PARSE_LOCATION_CITY_DISPLAY: dict[str, str] = {
...
def _resolve_parse_location_city(ville_raw: str) -> tuple[str, float, float]:
    key = ville_raw.strip().lower()
    if key in _PARSE_LOCATION_CITY_FALLBACK:
        lng, lat = _PARSE_LOCATION_CITY_FALLBACK[key]
        return _PARSE_LOCATION_CITY_DISPLAY.get(key, ville_raw.title()), lng, lat
    for city_key, coords in _PARSE_LOCATION_CITY_FALLBACK.items():
        if key in city_key or city_key in key:
            lng, lat = coords
            return _PARSE_LOCATION_CITY_DISPLAY.get(city_key, city_key.title()), lng, lat
    lng, lat = _PARSE_LOCATION_CITY_FALLBACK["genève"]
...
async def parse_location(
    ...
    """Parse NLP : ville + coords GPS + filtres extraits. Fallback local si Claude échoue."""
    ...
    ville_display, lng, lat = _resolve_parse_location_city(ville_raw)
