            ...
            <Marker key={b.id} position={[b.lat, b.lng]} icon={getPin(b.status)}>
              <Popup maxWidth={240}>
                <div style={{ fontFamily: "inherit", minWidth: 180 }}>
                  ...
                  {b.loyer && <p style={{ fontSize: 12, color: "#B55A30", fontWeight: 600, margin: "4px 0" }}>CHF {b.loyer.toLocaleString("fr-CH")}/mois</p>}
                  {b.locataire && <p style={{ fontSize: 11, color: "#7A7469", margin: "2px 0" }}>Locataire : {b.locataire}</p>}
                  <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                    <button
                      onClick={() => onBienClick?.(b.id)}
                      style={{ flex: 1, padding: "6px 0", borderRadius: 8, background: "#B55A30", border: "none", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}
                    >
                      Voir la fiche
                    </button>
                  </div>
