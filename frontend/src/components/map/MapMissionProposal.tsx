...
const missionPin = L.divIcon({
  ...
  html: `<svg width="36" height="45" viewBox="0 0 36 45" xmlns="http://www.w3.org/2000/svg">
    ...
    <path filter="url(#ms)" d="M18 1C10.8 1 5 6.8 5 14c0 10.3 13 30 13 30S31 24.3 31 14C31 6.8 25.2 1 18 1z" fill="#0F2E4C" stroke="white" stroke-width="1.5"/>
    <circle cx="18" cy="14" r="6" fill="white"/>
    <text x="18" y="18" text-anchor="middle" font-size="9" fill="#0F2E4C" font-weight="bold">M</text>
  </svg>`,
  iconSize: [36, 45],
  iconAnchor: [18, 45],
  popupAnchor: [0, -45],
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
        ...
        {([["distance", "Distance"], ["rating", "Note"], ["tarif", "Tarif"]] as [SortKey, string][]).map(([k, l]) => (
          <button key={k} onClick={() => setSortBy(k)} style={{ padding: "4px 12px", borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: "pointer", border: `1px solid ${sortBy === k ? "#0F2E4C" : "#E8E4DC"}`, background: sortBy === k ? "#D6E0ED" : "#fff", color: sortBy === k ? "#0F2E4C" : "#7A7469" }}>
            {l}
          </button>
        ))}
      </div>
      ...
      <div style={{ height, borderRadius: 14, overflow: "hidden", border: "1px solid #E8E4DC", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", position: "relative" }}>
          ...
          <Marker position={center} icon={missionPin}>
            <Popup>
              <div style={{ fontSize: 12 }}>
                <p style={{ fontWeight: 700, color: "#0F2E4C" }}>Point de mission</p>
          ...
          {mapOpeners.map((o, idx) => (
            <React.Fragment key={o.id}>
              ...
              >
                <Popup>
                  <div style={{ fontSize: 12, minWidth: 160 }}>
                    ...
                    {onSelect && (
                      <button onClick={() => onSelect(o.id)} style={{ marginTop: 8, width: "100%", padding: "6px 0", borderRadius: 8, background: "#0F2E4C", border: "none", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                        Proposer la mission
                      </button>
                    )}
                  </div>
              ...
              <Polyline
                ...
                pathOptions={{
                  color: o.id === selectedOpenerId ? "#0F2E4C" : o.is_available ? "#16A34A" : "#D1D5DB",
        ...
        <div style={{ position: "absolute", bottom: 10, left: 10, zIndex: 999, background: "rgba(255,255,255,0.92)", borderRadius: 10, padding: "8px 12px", fontSize: 11, backdropFilter: "blur(4px)", border: "1px solid #E8E4DC" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            <span style={{ width: 10, height: 10, borderRadius: "50%", background: "#0F2E4C", display: "inline-block" }} />
      ...
      {sorted.slice(0, 3).map((o, idx) => (
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "12px 14px", borderRadius: 12, background: selectedOpenerId === o.id ? "#D6E0ED" : "#FAFAF8", border: `1.5px solid ${selectedOpenerId === o.id ? "#0F2E4C" : "#E8E4DC"}`, cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}
        >
