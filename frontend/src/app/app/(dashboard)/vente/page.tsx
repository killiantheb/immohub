...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string }> = {
  actif:   { label: "Actif",     color: C.green,  bg: C.greenBg  },
  offre:   { label: "Offre",     color: C.orange, bg: C.orangeBg },
  vendu:   { label: "Vendu",     color: C.text3,  bg: C.surface2 },
  retire:  { label: "Retiré",    color: C.red,    bg: C.redBg    },
};

const OFFER_CFG: Record<string, { label: string; color: string }> = {
  recu:         { label: "Reçue",         color: C.orange },
  accepte:      { label: "Acceptée",      color: C.green  },
  refuse:       { label: "Refusée",       color: C.red    },
  contre_offre: { label: "Contre-offre",  color: C.amber  },
  expire:       { label: "Expirée",       color: C.text3  },
};
...
function EstimatePanel({ onMandateCreated }: { onMandateCreated: () => void }) {
  ...
  return (
    <div style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: 24 }}>
      <h3 style={{ margin: "0 0 4px", fontSize: 16, fontWeight: 700, color: C.text }}>Estimation IA gratuite</h3>
      <p style={{ margin: "0 0 20px", fontSize: 13, color: C.text3 }}>Obtenez une estimation en 30 secondes, sans engagement.</p>

      {/* Disclaimer */}
      <div style={{ display: "flex", gap: 8, padding: "10px 14px", backgroundColor: C.amberBg, border: `1px solid ${C.amber}`, borderRadius: 10, marginBottom: 20, fontSize: 12, color: C.amber }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
        ...
        <div>
          <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: C.text3, marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.06em" }}>Type de vente</label>
          <select value={form.mandate_type} onChange={e => setForm(p => ({ ...p, mandate_type: e.target.value }))}
            style={{ width: "100%", padding: "9px 12px", border: `1px solid ${C.border}`, borderRadius: 9, fontSize: 13.5, backgroundColor: C.surface2, color: C.text, outline: "none" }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: C.text3, marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.06em" }}>Description (optionnel)</label>
      ...
      <button
        ...
        style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 20px", backgroundColor: C.orange, color: "#fff", border: "none", borderRadius: 10, fontSize: 13.5, fontWeight: 700, cursor: estimateMut.isPending ? "default" : "pointer", opacity: estimateMut.isPending ? 0.7 : 1 }}
      >
      ...
      {/* Result */}
      {result && (
        <div style={{ marginTop: 20, padding: 20, backgroundColor: C.greenBg, border: `1px solid ${C.green}`, borderRadius: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
            {[
              ...
              <div key={k.label} style={{ textAlign: "center", padding: "12px 8px", backgroundColor: k.highlight ? C.green : "rgba(90,125,84,0.1)", borderRadius: 10 }}>
                <div style={{ fontSize: 18, fontWeight: 800, color: k.highlight ? "#fff" : C.green }}>{k.value}</div>
                <div style={{ fontSize: 10.5, color: k.highlight ? "rgba(255,255,255,0.8)" : C.text3, marginTop: 2 }}>{k.label}</div>
              </div>
            ))}
          </div>

          {form.mandate_type === "agency" && (
            <div style={{ padding: "10px 14px", backgroundColor: C.orangeBg, border: `1px solid ${C.orange}`, borderRadius: 10, marginBottom: 14, fontSize: 12.5, color: C.text }}>
          ...
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "none", color: C.green, fontSize: 12.5, fontWeight: 600, cursor: "pointer", marginBottom: showReport ? 10 : 0 }}
          >
            {showReport ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
            {showReport ? "Masquer" : "Voir"} l&apos;analyse complète
          </button>
          {showReport && (
            <pre style={{ margin: 0, fontSize: 12, color: C.text2, whiteSpace: "pre-wrap", lineHeight: 1.6 }}>{result.rapport}</pre>
          )}
          ...
          <div style={{ marginTop: 16, display: "flex", gap: 10 }}>
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 18px", backgroundColor: C.orange, color: "#fff", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer" }}
            >
              <Plus size={14} /> Créer le mandat
            </button>
            <button onClick={() => setResult(null)} style={{ padding: "10px 18px", border: `1px solid ${C.border}`, borderRadius: 10, backgroundColor: "transparent", color: C.text3, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              Recommencer
            </button>
          </div>
        </div>
...
function AddOfferModal({ mandateId, askingPrice, onClose }: { mandateId: string; askingPrice: number | null; onClose: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 60, backgroundColor: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ backgroundColor: C.surface, borderRadius: 20, padding: 28, width: "100%", maxWidth: 480, boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
        <h2 style={{ margin: "0 0 6px", fontSize: 17, fontWeight: 700, color: C.text }}>Enregistrer une offre</h2>
        {askingPrice && <p style={{ margin: "0 0 20px", fontSize: 13, color: C.text3 }}>Prix demandé : {fmt(askingPrice)}</p>}
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            ...
            <div key={key}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: C.text3, marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</label>
          ...
          <div>
            <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: C.text3, marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.06em" }}>Message (optionnel)</label>
            <textarea value={f.message} onChange={e => setF(p => ({ ...p, message: e.target.value }))} rows={2}
              style={{ width: "100%", padding: "9px 12px", border: `1px solid ${C.border}`, borderRadius: 9, fontSize: 13, backgroundColor: C.surface2, color: C.text, outline: "none", resize: "vertical", boxSizing: "border-box" }}
            />
          </div>
        </div>
...
function MandateDetail({ mandateId, onBack }: { mandateId: string; onBack: () => void }) {
  ...
  if (isLoading || !data) return <div style={{ padding: 40, textAlign: "center", color: C.text3 }}>Chargement…</div>;
  ...
  return (
    <div>
      ...
      <button onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: C.text3, fontSize: 13, cursor: "pointer", marginBottom: 20 }}>← Retour</button>

      {/* Mandate header */}
      <div style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: 24, marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <span style={{ fontSize: 18, fontWeight: 700, color: C.text }}>{mandate.address}, {mandate.city}</span>
            ...
            <div style={{ fontSize: 13, color: C.text3 }}>{mandate.surface_m2} m² · {mandate.nb_rooms} pièces{mandate.year_built ? ` · ${mandate.year_built}` : ""} · {mandate.mandate_type === "solo" ? "Vente solo" : "Via agence"}</div>
          </div>
          {mandate.status === "actif" && (
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowAddOffer(true)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "8px 14px", backgroundColor: C.orange, color: "#fff", border: "none", borderRadius: 9, fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}>
                <Plus size={13} /> Offre
              </button>
            </div>
          )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {[
            ...
            <div key={k.label} style={{ padding: "10px 14px", backgroundColor: C.surface2, borderRadius: 10 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: C.text }}>{k.value}</div>
        ...
        {mandate.ia_estimate_report && (
          <details style={{ marginTop: 14 }}>
            <summary style={{ fontSize: 12.5, color: C.orange, cursor: "pointer", fontWeight: 600 }}>Rapport IA complet</summary>
            <pre style={{ margin: "8px 0 0", fontSize: 11.5, color: C.text3, whiteSpace: "pre-wrap", lineHeight: 1.6 }}>{mandate.ia_estimate_report}</pre>
          </details>
        )}
      </div>
      ...
      <div style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
        <div style={{ padding: "16px 22px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: C.text }}>Offres reçues ({offers.length})</h3>
          {mandate.status === "offre" && (
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 14px", backgroundColor: C.greenBg, color: C.green, border: `1px solid ${C.green}`, borderRadius: 9, fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}
            >
        ...
        {offers.length === 0 ? (
          <div style={{ padding: "30px 22px", textAlign: "center", color: C.text3, fontSize: 13 }}>Aucune offre enregistrée pour l&apos;instant</div>
        ) : (
          <div>
            {offers.map(o => {
              const oc = OFFER_CFG[o.status] ?? OFFER_CFG.recu;
              return (
                <div key={o.id} style={{ padding: "16px 22px", borderBottom: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                        <span style={{ fontSize: 16, fontWeight: 800, color: C.text }}>{fmt(o.offer_price)}</span>
                      ...
                      {o.buyer_name && <div style={{ fontSize: 12.5, color: C.text3 }}>👤 {o.buyer_name}{o.buyer_email ? ` · ${o.buyer_email}` : ""}{o.buyer_phone ? ` · ${o.buyer_phone}` : ""}</div>}
                      {o.message && <div style={{ fontSize: 12, color: C.text3, marginTop: 4, fontStyle: "italic" }}>{o.message}</div>}
                      {o.counter_offer_price && <div style={{ fontSize: 12.5, color: C.amber, marginTop: 4 }}>Contre-offre : {fmt(o.counter_offer_price)}</div>}
                    </div>
                    {o.status === "recu" && (
                      <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                        <button onClick={() => actionMut.mutate({ offerId: o.id, action: "accept" })} style={{ padding: "6px 12px", backgroundColor: C.greenBg, color: C.green, border: `1px solid ${C.green}`, borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Accepter</button>
                        <button onClick={() => actionMut.mutate({ offerId: o.id, action: "refuse" })} style={{ padding: "6px 12px", backgroundColor: C.redBg, color: C.red, border: `1px solid ${C.red}`, borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Refuser</button>
                        <div style={{ display: "flex", gap: 4 }}>
                          <input
                            ...
                            style={{ width: 130, padding: "6px 10px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12, backgroundColor: C.surface2, color: C.text, outline: "none" }}
                          />
                          <button
                            ...
                            style={{ padding: "6px 10px", backgroundColor: C.amberBg, color: C.amber, border: `1px solid ${C.amber}`, borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}
                          >↩</button>
                        </div>
                      </div>
                    )}
      ...
      <div style={{ marginTop: 16, padding: "12px 16px", backgroundColor: C.surface2, borderRadius: 12, fontSize: 12.5, color: C.text3 }}>
  ...
  return (
    <div style={{ padding: "28px 24px", maxWidth: 1000, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <h1 style={{ margin: "0 0 6px", fontSize: 26, fontWeight: 700, color: C.text, letterSpacing: "-0.02em" }}>Vente immobilière</h1>
          <p style={{ margin: 0, color: C.text3, fontSize: 13.5 }}>Estimation IA · Mandats · Offres · Référence notaire</p>
        </div>
        <button
          onClick={() => setShowEstimate(v => !v)}
          style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 18px", backgroundColor: C.orange, color: "#fff", border: "none", borderRadius: 10, fontSize: 13.5, fontWeight: 700, cursor: "pointer" }}
        >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 28 }}>
        {[
          { label: "Mandats actifs",   value: activeCount,         color: C.orange },
          { label: "Offres en cours",  value: offerCount,          color: C.amber  },
          { label: "Biens vendus",     value: soldCount,           color: C.green  },
          { label: "Volume total",     value: fmt(totalVolume),    color: C.text   },
        ].map(k => (
      ...
      {isLoading ? (
        ...
        <div style={{ textAlign: "center", padding: "60px 20px", backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 16 }}>
          ...
          <h3 style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 600, color: C.text }}>Aucun mandat de vente</h3>
          <p style={{ color: C.text3, fontSize: 13, margin: "0 0 20px" }}>Commencez par obtenir une estimation IA gratuite pour votre bien.</p>
          <button onClick={() => setShowEstimate(true)} style={{ padding: "10px 24px", backgroundColor: C.orange, color: "#fff", border: "none", borderRadius: 10, fontSize: 13.5, fontWeight: 700, cursor: "pointer" }}>
            Estimer mon bien
          </button>
        </div>
      ) : (
          ...
          {mandates.map(m => {
            ...
            return (
              <div
                ...
                style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "16px 20px", cursor: "pointer", boxShadow: C.shadow, transition: "box-shadow 0.15s" }}
              >
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <Home size={14} color={C.text3} />
                      <span style={{ fontSize: 14.5, fontWeight: 700, color: C.text }}>{m.address}, {m.city}</span>
                      <span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10.5, fontWeight: 600, backgroundColor: sc.bg, color: sc.color }}>{sc.label}</span>
                      {m.mandate_type === "agency" && (
                        <span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10.5, fontWeight: 600, backgroundColor: C.surface2, color: C.text3 }}>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 17, fontWeight: 800, color: C.text }}>{fmt(m.asking_price ?? m.ia_estimate)}</div>
                    {delta !== null && (
                      <div style={{ fontSize: 11.5, color: Math.abs(delta) > 10 ? C.amber : C.text3 }}>
