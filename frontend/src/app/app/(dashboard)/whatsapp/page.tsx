...
export default function WhatsAppPage() {
  ...
  // ── Supabase Realtime: listen for new WhatsApp messages ───────────────────
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Refresh conversation list so unread counts stay accurate
          loadConversations();

          // Append to open conversation in real-time
          ...
          // Auto-generate AI suggestion when a new inbound message arrives
  ...
  return (
    <div style={{ maxWidth: 1020, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: 28, fontWeight: 300, color: "var(--althy-text)", margin: 0 }}>
          WhatsApp
        </h1>
        <p style={{ fontSize: 13, color: "var(--althy-text-3)", marginTop: 4 }}>
          Messages avec vos locataires, artisans et ouvreurs
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ ...S.card, textAlign: "center", padding: "56px 24px" }}>
          ...
          <p style={{ fontSize: 13, color: "var(--althy-text-3)", maxWidth: 440, margin: "0 auto 20px", lineHeight: 1.6 }}>
            Vos conversations WhatsApp avec vos locataires, artisans et ouvreurs apparaîtront ici.
            Configurez votre numéro WhatsApp Business dans les paramètres pour activer cette fonctionnalité.
          </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 0, ...S.card, overflow: "hidden", height: "calc(100vh - 200px)", minHeight: 500 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              {aiSuggestion && (
                <div style={{ margin: "0 16px 8px", padding: "10px 14px", background: "var(--althy-orange-bg, rgba(232,96,44,0.08))", border: "1px solid rgba(232,96,44,0.2)", borderRadius: 10, display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <p style={{ flex: 1, margin: 0, fontSize: 12, color: "var(--althy-text)", lineHeight: 1.5 }}>
                    {aiSuggestion}
                  </p>
                  <button
                    onClick={() => { setInput(aiSuggestion); setAiSuggestion(null); }}
                    style={{ padding: "5px 12px", borderRadius: 8, border: "none", background: "var(--althy-orange)", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}
                  >
                    Utiliser
                  </button>
                  <button
                    onClick={() => setAiSuggestion(null)}
                    style={{ padding: "5px 8px", borderRadius: 8, border: "1px solid var(--althy-border)", background: "transparent", cursor: "pointer", fontSize: 11, color: "var(--althy-text-3)" }}
                  >
                    ✕
                  </button>
                </div>
