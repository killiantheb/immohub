...
function InvitationsTab() {
  ...
  if (selectedInv) {
    return (
      <div>
        <button onClick={() => setSelectedInv(null)} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: C.text3, fontSize: 13, cursor: "pointer", marginBottom: 16 }}>← Retour</button>
        <div style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
          <div style={{ padding: "14px 18px", borderBottom: `1px solid ${C.border}`, backgroundColor: C.surface2 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{selectedInv.proprio_name ?? selectedInv.proprio_email}</div>
            <div style={{ fontSize: 12, color: C.text3 }}>
              Portail : <code style={{ fontSize: 11, backgroundColor: C.bg, padding: "1px 6px", borderRadius: 4 }}>/portail/{selectedInv.token}</code>
              <button onClick={() => navigator.clipboard.writeText(`${window.location.origin}/portail/${selectedInv.token}`)} style={{ marginLeft: 8, padding: "2px 8px", border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 11, backgroundColor: "transparent", color: C.text3, cursor: "pointer" }}>Copier le lien</button>
            </div>
          </div>
          {/* Messages */}
          <div style={{ padding: 16, minHeight: 200, maxHeight: 360, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8 }}>
            {messages.length === 0 && <div style={{ textAlign: "center", padding: "30px 0", color: C.text3, fontSize: 13 }}>Aucun message pour l&apos;instant</div>}
            {messages.map(m => (
              <div key={m.id} style={{ display: "flex", justifyContent: m.sender_type === "agency" ? "flex-end" : "flex-start" }}>
                <div style={{
                  ...
                  backgroundColor: m.sender_type === "agency" ? C.orange : m.sender_type === "ai" ? C.amberBg : C.surface2,
                  color: m.sender_type === "agency" ? "#fff" : C.text,
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <p style={{ margin: 0, fontSize: 13, color: C.text3 }}>Invitations token CHF 9/mois — le proprio accède à son portail sans créer de compte.</p>
        <button onClick={() => setShowForm(v => !v)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", backgroundColor: C.orange, color: "#fff", border: "none", borderRadius: 9, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          <PlusCircle size={14} /> Inviter un proprio
        </button>
      </div>

      {showForm && (
        <div style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
            ...
            {[
              ...
              <div key={key} style={{ gridColumn: col ?? "auto" }}>
                ...
                <input value={(form as Record<string, string>)[key]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} placeholder={placeholder}
                  style={{ width: "100%", padding: "9px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 13, backgroundColor: C.bg, color: C.text, outline: "none", boxSizing: "border-box" }}
                />
              </div>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => createInv.mutate()} disabled={!form.proprio_email || createInv.isPending}
              style={{ padding: "9px 18px", backgroundColor: C.orange, color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              {createInv.isPending ? "Création…" : "Créer l'invitation"}
            </button>
            <button onClick={() => setShowForm(false)} style={{ padding: "9px 18px", border: `1px solid ${C.border}`, borderRadius: 8, backgroundColor: "transparent", color: C.text3, fontSize: 13, cursor: "pointer" }}>Annuler</button>
          </div>
        </div>
      )}

      {isLoading ? (
        <div style={{ textAlign: "center", padding: 30, color: C.text3 }}>Chargement…</div>
      ) : invitations.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 20px", backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 14 }}>
          <Link2 size={28} style={{ color: C.text3, marginBottom: 10 }} />
          <div style={{ fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 4 }}>Aucune invitation envoyée</div>
          <div style={{ fontSize: 13, color: C.text3 }}>Invitez vos propriétaires mandants pour leur donner accès à leur portail.</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {invitations.map(inv => (
            <div key={inv.id} style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{inv.proprio_name ?? inv.proprio_email}</div>
                <div style={{ fontSize: 12, color: C.text3 }}>{inv.proprio_email}</div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10.5, fontWeight: 600, backgroundColor: inv.inv_status === "active" ? C.greenBg : C.surface2, color: inv.inv_status === "active" ? C.green : C.text3 }}>
                  {inv.inv_status === "active" ? <><CheckCircle2 size={10} style={{ display: "inline", marginRight: 3 }} />Actif</> : "En attente"}
                </span>
                <button onClick={() => setSelectedInv(inv)} style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", border: `1px solid ${C.border}`, borderRadius: 8, backgroundColor: "transparent", color: C.text2, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 28, display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontFamily: "var(--font-serif)", fontSize: 28, fontWeight: 300, color: C.text, margin: "0 0 6px" }}>
      ...
      <div style={{ display: "flex", gap: 2, borderBottom: `1px solid ${C.border}`, marginBottom: 24 }}>
        {([
          ...
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 16px", background: "none", border: "none", borderBottom: `2px solid ${activeTab === t.id ? C.orange : "transparent"}`, color: activeTab === t.id ? C.orange : C.text3, fontWeight: activeTab === t.id ? 700 : 500, fontSize: 13, cursor: "pointer" }}>
            {t.icon}{t.label}
          </button>
        ))}
      </div>
      ...
      {showForm && (
        <div style={{
              ...
              <div key={field}>
                ...
                <input
                  ...
                  style={{
                    ...
                    color: C.text, background: "var(--cream)",
                  }}
                />
              </div>
            ))}
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, color: C.text3, display: "block", marginBottom: 4 }}>Email</label>
            <input
              type="email"
              value={form.email}
      ...
      {/* Users list */}
      {isLoading ? (
        <div style={{ color: C.text3, fontSize: 13.5, padding: 20 }}>Chargement…</div>
      ) : users.length === 0 ? (
        <div style={{
          background: C.surface, border: `1px solid ${C.border}`,
          ...
          <Users2 size={32} style={{ color: C.text3, marginBottom: 12 }} />
            ...
            <div key={u.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12.5, color: C.text3 }}>{u.email}</div>
                </div>
              </div>
