...
function ProviderCard({ status, onConnect, onDisconnect }: {
  status: OAuthStatus | null;
  provider: "google" | "microsoft";
  onConnect: () => void;
  onDisconnect: () => void;
}) {
  const isGoogle = !status || status.provider === "google";
  const icon = isGoogle ? "📧" : "📩";
...
export default function MessageriePage() {
  ...
  async function handleConnect(provider: "google" | "microsoft") {
    try {
      ...
      const res = await fetch(`${baseURL}/oauth/${provider}/autoriser`, {
      ...
      if (data.auth_url) {
        ...
        alert(data.detail ?? "Impossible de lancer la connexion OAuth");
      }
    } catch {
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--althy-text-3)", marginTop: 4 }}>
            Emails immobiliers détectés et analysés par Althy
          </p>
        </div>
        {isConnected && (
          <button
      ...
      {isConnected ? (
        ...
        <div style={{ ...S.card, textAlign: "center", padding: "40px 24px" }}>
          ...
          <p style={{ fontSize: 13, color: "var(--althy-text-3)", maxWidth: 420, margin: "0 auto 24px" }}>
            Althy analyse vos emails immobiliers et crée automatiquement des actions dans la Sphère — relances, rendez-vous, devis.
          </p>
