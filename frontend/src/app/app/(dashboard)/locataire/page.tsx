...
const STATUT_STYLE = {
  ok:      { label: "Loyer à jour",  bg: C.greenBg, fg: C.green },
  retard:  { label: "En retard",     bg: C.amberBg, fg: C.amber },
  impaye:  { label: "Impayé",        bg: C.redBg,   fg: C.red   },
};
...
export default function LocatairePage() {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: "0 auto" }}>
      ...
      {/* Search */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 14px", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, marginBottom: 20 }}>
      ...
      {isLoading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(l => {
            ...
            const href = l.bien_id
              ? `/app/biens/${l.bien_id}/historique/${l.id}`
              : undefined;

            const card = (
              <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, cursor: href ? "pointer" : "default", transition: "box-shadow 0.15s" }}
                onMouseEnter={e => { if (href) (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 12px rgba(26,22,18,0.08)"; }}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {l.bien_adresse && (
                    <p style={{ margin: "2px 0 0", fontSize: 12, color: C.text3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                ...
                {l.loyer_mensuel != null && (
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: C.orange }}>
                      CHF {l.loyer_mensuel.toLocaleString("fr-CH")}
                    </p>
                    <p style={{ margin: 0, fontSize: 10, color: C.text3 }}>/ mois</p>
                  </div>
                )}
            ...
            return href ? (
              <Link key={l.id} href={href} style={{ textDecoration: "none", color: "inherit" }}>
                {card}
              </Link>
            ) : (
              <div key={l.id} title="Détail bientôt disponible">
                {card}
              </div>
            );
          })}
