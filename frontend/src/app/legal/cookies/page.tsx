  ...
  return (
    <div style={{ marginBottom: "1.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.75rem" }}>
        ...
        <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--althy-text)", margin: 0 }}>{title}</h3>
      </div>
          ...
          <thead>
            <tr style={{ background: "var(--althy-surface-2)" }}>
              {["Nom / Identifiant", "Fournisseur", "Finalité", "Durée"].map(h => (
                <th key={h} style={{ padding: "7px 10px", textAlign: "left", color: "var(--althy-text-3)", fontWeight: 600, borderBottom: "1px solid var(--althy-border)" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {cookies.map((c, i) => (
              <tr key={c.name} style={{ borderBottom: "1px solid var(--althy-border)", background: i % 2 === 0 ? "#fff" : "var(--althy-bg)" }}>
                <td style={{ padding: "7px 10px", color: "var(--althy-text)", fontWeight: 600 }}>{c.name}</td>
                <td style={{ padding: "7px 10px", color: "var(--althy-text)" }}>{c.provider}</td>
                <td style={{ padding: "7px 10px", color: "var(--althy-text)" }}>{c.purpose}</td>
                <td style={{ padding: "7px 10px", color: "var(--althy-text-3)", whiteSpace: "nowrap" as const }}>{c.duration}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function CookiesPage() {
  return (
    <>
      ...
      <div style={{ padding: "1rem", background: "var(--althy-green-bg)", borderRadius: 10, border: "1px solid #A8C9A4", fontSize: 13 }}>
      ...
      <ul style={{ paddingLeft: "1.5rem", margin: "0.5rem 0" }}>
        <li style={{ fontSize: 14, lineHeight: 1.75, color: "var(--althy-text)", marginBottom: "0.25rem" }}>
