  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060402', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2rem 1.2rem', fontFamily: 'var(--font-sans)' }}>
      ...
      <div style={{ width: '100%', maxWidth: 380, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <span style={{ fontSize: 10, letterSpacing: 1, textTransform: 'uppercase', color: 'rgba(200,95,25,0.35)', padding: '4px 10px', borderRadius: 20, border: '0.5px solid rgba(212,96,26,0.12)' }}>Locataire</span>
      </div>
      ...
      <div style={{ width: '100%', maxWidth: 380, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {/* Prochain loyer */}
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 16, padding: '20px', border: '0.5px solid rgba(212,96,26,0.15)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '1.5px', color: 'rgba(200,95,25,0.45)' }}>Prochain loyer</span>
            {info && (
          ...
          {loading ? (
            ...
            <>
              ...
              <div style={{ fontSize: 12, color: 'rgba(200,150,80,0.6)' }}>
                Échéance le {formatDate(info.next_rent_due)}
              </div>
              {info.property_address && (
                <div style={{ fontSize: 11, color: 'rgba(200,150,80,0.4)', marginTop: 4 }}>{info.property_address}</div>
              )}
            </>
          ) : (
            <div style={{ fontSize: 13, color: 'rgba(200,150,80,0.4)' }}>Information indisponible</div>
          )}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <button
            ...
            style={{ padding: '14px 0', borderRadius: 14, border: '0.5px solid rgba(212,96,26,0.2)', background: 'rgba(212,96,26,0.05)', color: 'rgba(255,195,140,0.8)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}
          >
        ...
        {/* Bail info */}
        {info?.lease_end_date && (
          <div style={{ background: 'rgba(255,255,255,0.02)', borderRadius: 12, padding: '12px 14px', border: '0.5px solid rgba(212,96,26,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: 'rgba(200,150,80,0.5)', letterSpacing: 1 }}>Fin du bail</span>
            <span style={{ fontSize: 12, color: 'rgba(255,195,140,0.7)' }}>{formatDate(info.lease_end_date)}</span>
          </div>
        )}
      </div>
